...
class UsuarioDAO {
    ...
    function salvar($usuario, $conn){
        ...
        if ($conn->query($sql) == TRUE){
            $this->login($usuario->getEmail(), $usuario->getSenha(), $conn);
        }
    ...
    function alterar($conn, $id, $usuario){
        ...
        if ($conn->query($sql) == TRUE){
            ...
            if ($conn->errno == 1062) {
                echo "<script type='text/javascript'>alert('Este usuário já possui cadastro!');
                                                     location.href='C_meusDadosEstudante.php'</script>";
            }
            else{
            ...
            if ($usuario == $userBD['email']){
                ...
                if (mysqli_num_rows($senhaBD) >= 1){
                    ...
                    if($senha == $passwordBD['senha']){
                        $sql = "SELECT id, nome, tipo FROM usuario WHERE email= '".$usuario."'";
                        $res = $conn->query($sql);
                        ...
                        $_SESSION['nome'] = $nome;
                        if ($linha['tipo'] == 'f'){
                            echo "<script>location.href='../Controller/C_funcionario.php'</script>";
                        }
                        else if($linha['tipo'] == 'e'){
                            echo "<script>location.href='../Controller/C_estudante.php'</script>";
                        }
                    }
                }
                else {
